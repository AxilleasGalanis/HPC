...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	//1: Loop interchange
	for (i=1; i<SIZE-1; i+=2) {//3:Loop unrolling
		for (j=1; j<SIZE-1; j+=1 ) {
			 ...
			 * pixel we process.
													*/
			//2: Loop interchange, function inling, loop fusion								  
			int m, n, conv_horiz = 0, conv_vert = 0, conv_horiz_1 = 0, conv_vert_1 = 0;
			for (m = -1; m <= 1; m++) {
				for (n = -1; n <= 1; n++) {
					conv_horiz += input[(i + m)*SIZE + j + n] * horiz_operator[m+1][n+1];
					conv_horiz_1 += input[(i+1 + m)*SIZE + j + n] * horiz_operator[m+1][n+1]; //3:Loop unrolling
					conv_vert += input[(i + m)*SIZE + j + n] * vert_operator[m+1][n+1];
					conv_vert_1 += input[(i+1 + m)*SIZE + j + n] * vert_operator[m+1][n+1];	//3:Loop unrolling
				}
			}
			...
			p1 = pow(conv_horiz_1, 2) + 
				pow(conv_vert_1, 2);	//3:Loop unrolling	
				
				
			res = (int)sqrt(p);
			res1 = (int)sqrt(p1);	//3:Loop unrolling
